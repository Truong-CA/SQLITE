...
def them():
    ...
    c = conn.cursor()
    # Lấy dữ liệu đã nhập
    id_student_value = id_st.get()
    firstname_value =f_name.get()
    lastname_value = l_name.get()
    id_class_value = id_cl.get()
    state_value = state.get()
    gpa_score_value = gpa.get()
    # Thực hiện câu lệnh để thêm
    c.execute('''
        INSERT INTO 
        sinhvien (id_student,first_name, last_name, id_class , state, gpa_score)
        VALUES 
        (:id_st:first_name,:last_name,:id_class,:state,:gpa)
    ''',{
        'id_st': id_student_value,
        'first_name' :firstname_value,
        'last_name' : lastname_value,
        'id_class': id_class_value,
        'state_value ': state_value,
        'gpa_score_value': gpa_score_value,
...
def chinh_sua():
    ...
    c.execute("SELECT * FROM sinhvien WHERE id=:id", {'id':record_id})
    records = c.fetchall()

    global f_id_editor,id_st_editor, f_name_editor, l_name_editor, id_cl_editor, state_editor, gpa_editor

    f_id_editor = Entry(editor, width=30)
    f_id_editor.grid(row=0, column=1, padx=20, pady=(10, 0))
    id_st_editor = Entry(editor, width=30)
    id_st_editor.grid(row=0, column=1, padx=20)
    f_name_editor = Entry(editor, width=30)
    f_name_editor.grid(row=1, column=1, padx=20)
    l_name_editor = Entry(editor, width=30)
    l_name_editor.grid(row=2, column=1)
    id_cl_editor = Entry(editor, width=30)
    id_cl_editor.grid(row=3, column=1)
    state_editor = Entry(editor, width=30)
    state_editor.grid(row=5, column=1)
    gpa_editor = Entry(editor, width=30)
    gpa_editor.grid(row=6, column=1)


    f_id_label = Label(editor, text="ID")
    f_id_label.grid(row=0, column=0, pady=(10, 0))
    id_st_label = Label(editor, text="Mã sinh viên")
    id_st_label.grid(row=0, column=0)
    f_name_label = Label(editor, text="Họ")
    f_name_label.grid(row=1, column=0)
    l_name_label = Label(editor, text="Tên")
    l_name_label.grid(row=2, column=0)
    id_cl_label = Label(editor, text="Mã lớp")
    id_cl_label.grid(row=3, column=0)
    state_label = Label(editor, text="Năm nhập học")
    state_label.grid(row=4, column=0)
    gpa_label = Label(editor, text="Điểm trung bình")
    gpa_label.grid(row=6, column=0)

    for record in records:
        f_id_editor.insert(0, record[0])
        id_st_editor.insert(0, record[1])
        f_name_editor.insert(0, record[2])
        l_name_editor.insert(0, record[3])
        id_cl_editor.insert(0, record[4])
        state_editor.insert(0, record[5])
        gpa_editor.insert(0, record[6])
...
def cap_nhat():
    ...
    c.execute("""UPDATE addresses SET
           ...
           WHERE id = :id""",
              {
                  'id_student': id_st_editor.get(),
                  'first': f_name_editor.get(),
                  'last': l_name_editor.get(),
                  'id_class': id_cl_editor.get(),
                  'state': state_editor.get(),
                  'avg_score': gpa_editor.get(),
                  'id': record_id
              })
...
id_st= Entry(input_frame, width=30)
id_st.grid(row=1, column=1, padx=20, pady=(10, 0))
...
id_cl = Entry(input_frame, width=30)
id_cl.grid(row=4, column=1)
state = Entry(input_frame, width=30)
state.grid(row=5, column=1)
gpa = Entry(input_frame, width=30)
gpa.grid(row=6, column=1)

# Các nhãn
id_st_label = Label(input_frame, text="Mã sinh viên")
id_st_label.grid(row=1, column=0, pady=(10, 0))
...
id_cl_label = Label(input_frame, text="Mã lớp")
id_cl_label.grid(row=4, column=0)
state_label = Label(input_frame, text="Năm nhập học")
state_label.grid(row=5, column=0)
gpa_label = Label(input_frame, text="Điểm trung bình")
gpa_label.grid(row=6, column=0)
