...
def them():
    ...
    conn = sqlite3.connect('stb.db')
    c = conn.cursor()
    # Lấy dữ liệu đã nhập
    masinhvien_value =msv.get()
    ...
    c.execute('''
        INSERT INTO 
        qlsv (masv, first_name, last_name, malop, state,gpa)
        VALUES 
        (:msv ,:name, :last_name, ::malop, :state, :gpa )
    ''',{
        'masv': masinhvien_value,
        'name' : name_value,
...
def chinh_sua():
    ...
    global f_id_editor,msv_editor, f_name_editor, l_name_editor, malop_editor, state_editor, gpa_editor

    f_id_editor = Entry(editor, width=30)
    f_id_editor.grid(row=0, column=1, padx=20, pady=(10, 0))
    msv_editor = Entry(editor, width=30)
    msv_editor.grid(row=1, column=1, padx=20)
    ...
    msv_label = Label(editor, text="Mã sinh viên")
    msv_label.grid(row=1, column=0)
